...
class LaneDetection:
    ...
    # == Img Processing == 
    # 1. Gaussian Blur는 Edge까지 지워버림 -> 양방향 필터(Bilateral Filter)로 바꿈
    # 2. Color selection - white, yellow
    ...
    def roi(self, frame):
        ...
        bottom_left = [0, height * 0.95]
        top_left = [width * 0.25, height * 0.6]
        bottom_right = [width, height * 0.95]
        top_right = [width * 0.7, height * 0.6]
        
        self.vertices = np.array([[bottom_left, top_left, top_right, bottom_right]],
                                  dtype=np.int32) 
    ...
    def bed(self, frame):
        ...
        src_points = np.float32([bottom_left, top_left,
                                  top_right, bottom_right])
        dst_points = np.float32([[0, height], [0, 0],
...
def main():
    video_path = r"C:\Users\User\Desktop\MORARI 4월부터 차선검출 코드\CrossWalk\output_video1280-720.mp4"
